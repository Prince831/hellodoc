...
serve(async (req) => {
  ...
  try {
    const { doctorId, profileData } = await req.json();

    if (!doctorId || !profileData) {
      return new Response(JSON.stringify({ error: 'Missing doctorId or profileData' }), {
        status: 400,
    ...
    return new Response(JSON.stringify({ updated: true, data }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
